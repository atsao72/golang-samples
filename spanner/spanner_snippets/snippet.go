...
func write(ctx context.Context, w io.Writer, client *spanner.Client) error {
        ...
        m := []*spanner.Mutation{
                //spanner.InsertOrUpdate("Singers", singerColumns, []interface{}{time.Now().UnixNano(), "David", "Lomond"}),
                spanner.InsertOrUpdate("Singers", singerColumns, []interface{}{1, "David", "Lomond"}),
        }
...
func run(ctx context.Context, adminClient *database.DatabaseAdminClient, dataClient *spanner.Client, w io.Writer, cmd string, db string) error {
        ...
        for i := 0; i < 100; i++ {
                ...
                start = time.Now()
                err = read(ctx, w, dataClient)
                if err != nil {
                        fmt.Fprintf(w, "%s failed with %v", "read", err)
                }
                elapsed = time.Since(start)
        ...
        log.Printf("90th Percentile %s: %s", cmd, times[89])
        log.Printf("98th Percentile %s: %s", cmd, times[97])
        log.Printf("Median %s: %s", "read", readTimes[len(readTimes)/2])
        log.Printf("90th Percentile %s: %s", "read", readTimes[89])
        log.Printf("98th Percentile %s: %s", "read", readTimes[97])
